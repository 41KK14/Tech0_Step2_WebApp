...
if uploaded_image:
    ...
    # アップロード画像を全体が写るように縦346px × 横346pxの正方形にリサイズして中央に貼り付け
    ...
    # 中央下に PIC_5_1.png を表示
    corner_path = os.path.join("assets", "PIC_5_1.png")
    ...
    if selected_msg not in [None, "↓Message↓"]:
        msg = selected_msg
        image_path = os.path.join("assets", f"{get_message_image_filename(msg)}.png")
        if os.path.exists(image_path):
            ...
            paste_x = 10
            paste_y = 80
            # ステッカー画像がテンプレートをはみ出さないようにリサイズ（任意）
    ...
    draw.text((x, y), today_str, font=font, fill=(0, 0, 0))
...
# 表示と選択対象を分離
all_messages = []
for msgs in message_options.values():
